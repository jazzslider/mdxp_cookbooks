define :drush_site_install, :action => :install, :dir => nil, :profile => "default", :account_name => nil, :account_pass => nil, :account_mail => nil, :locale => nil, :clean_url => nil, :site_name => nil, :site_mail => nil, :sites_subdir => nil do
  case params[:action]
  when :install
    ...
    drush_command = "#{node[:drupal][:drush][:dir]}/drush site-install #{params[:profile]}"
    ...
    # Only run the install if drush pm-list fails; otherwise, we've
    # already got a functional database.
    execute drush_command do
      cwd params[:dir]
      user "www-data"
      group "www-data"
      not_if "#{node[:drupal][:drush][:dir]}/drush pm-list"
    end
  else
